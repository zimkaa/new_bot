...
from settings import STATE_FILE_TEST
from settings import STATE_FILE_SIMPLE
from settings import STORAGE_FILE
from settings import STORAGE_FILE_TEST
from settings import STORAGE_FILE_SIMPLE
...
def reed_store_simple(coin_name: str) -> dict:
...
def read_store_sell_price_simple(coin_name: str) -> Decimal:
    file_name = STORAGE_FILE_SIMPLE
    with open(file_name, "r", encoding="utf8") as my_coins_data:
        my_coins = json.loads(my_coins_data.read())
    return Decimal(my_coins[coin_name]["sellPrice"])
...
def read_store_sell_time_simple(coin_name: str) -> str:
    file_name = STORAGE_FILE_SIMPLE
    with open(file_name, "r", encoding="utf8") as my_coins_data:
        my_coins = json.loads(my_coins_data.read())
    return my_coins[coin_name]["sellTime"]
...
def write_state_simple(coin_name: str, data: bool, fall: Optional[Decimal] = None):
    ...
    try:
        ...
        with open(file_name, "r+", encoding="utf8") as my_coins_data:
            ...
            if type_operation == TradeStatus.BUY:
                ...
                stop_loss_price = coin_price * 1.01
                ...
                try:
                    ...
                    data_base_simple.add(Balance(coin_name=coin_name, balance=bl, result_value=profit))
                    data_base_simple.commit()
        ...
        data_base_simple.add(
            History(coin_name=coin_name, amount=amount, operation_type=type_operation, price=coin_price)
        )
        data_base_simple.commit()
        return True
